...
def _sweepAnalyzer(data, tim, SNR, minr=0.05, maxd=0.004, mdc=0.01, mdc2=0.1, cis=3, dr=5, rr=30, ch=12,
                   plot=False):  # min_requirement,max_derivative,max_derivative_contact, max_derivative_contact2,contact_index_security,derivative_range,regression_range
    gc.enable()
    ...
    if plot and afterflag:
        pyplot.plot(regtimafter, regdataafter[:, ch] + SCALE, color='#addd8e', marker='.', linestyle='',
                    label='interpolation data (after) v1')
        pyplot.plot(timafter[contact], dataafter[contact, ch] + SCALE, color='#addd8e', marker='x', markersize=10,
                    label='contact_point (after) v1')
    ...
    if plot and beforeflag:
        pyplot.plot(regtimbefore, regdatabefore[:, ch] + SCALE, color='#c994c7', marker='.', linestyle='',
                    label='interpolation data (before) v1')
        pyplot.plot(timbefore[contact], databefore[contact, ch] + SCALE, color='#c994c7', marker='x', markersize=10,
                    label='contact_point (before) v1')
    ...
    if beforeflag and len(regdatabefore[:, 0]) > 3:
        ...
        beforeflag = False
    if plot and beforeflag:
        pyplot.plot(tim, ((tim - stim) * slope[ch] + intercept[ch]) + SCALE, color='#c994c7', linestyle=':',
                    label='linear regression (before) v1')
    ...
    if plot and afterflag:
        pyplot.plot(tim, ((tim - stim) * slope[ch] + intercept[ch]) + SCALE, color='#addd8e', linestyle=':',
                    label='linear regression (after) v1')
...
def _sweepAnalyzer2(data, tim, SNR, minr=0.05, maxd=0.004, mdc=0.01, mdc2=0.1, cis=3, dr=1, rr=240,
                    ch=12):  # min_requirement,max_derivative,max_derivative_contact, max_derivative_contact2,contact_index_security,derivative_range,regression_range
    gc.enable()
    ...
    if beforeflag and afterflag and len(regdataafter[:, 0]) > 3 and len(regdatabefore[:, 0]) > 3:
        ...
        for i in range(CH):
            ...
            #
            popt, pconv = curve_fit(_line, regtimbefore - mintim, regdatabefore[:, i],
                                    sigma=regnoisebefore[:, i])  # [:-30]
            slope, inter = popt
...
def _sweepAnalyzer3(data, tim, SNR, Cmin=0.05, Cder=0.01, Mdc=0.05, dr=1, rr=30, ch=12,
                    plot=False):  # dr=1 rr=30  rr=40 dr=5
    gc.enable()
    ...
    if contact:
        ...
        if plot and AFTER_flag:
            pyplot.plot(AFTER_Itim, AFTER_Idata[:, ch], color='#31a354', marker='.', linestyle='',
                        label='interpolation data (after)')
            pyplot.plot(AFTER_tim[0], AFTER_data[0, ch], color='#31a354', marker='x', markersize=10,
                        label='contact_point (after)')
    #            for i in range(CH-1):
    ...
    if np.count_nonzero(ind) < rr * 0.7:
        return np.nan, [np.nan] * CH, [np.nan] * CH, [np.nan] * CH, [np.nan] * CH
    # derivative
    der = np.abs((data[ind, ch][:-1][::-1] - data[ind, ch][1:][::-1]) / (
                0.5 * (data[ind, ch][1:][::-1] + data[ind, ch][:-1][::-1])))
    der = griddata(tim[ind][1:][::-1] + 0.5 * np.mean(tim[ind][:-1][::-1] - tim[ind][1:][::-1]), der, tim[ind][::-1],
                   fill_value=-np.inf)
    #    if plot :
    ...
    if contact:
        ...
        if plot and BEFOR_flag:
            pyplot.plot(BEFOR_Itim, BEFOR_Idata[:, ch], color='#dd1c77', marker='.', linestyle='',
                        label='interpolation data (before)')
            pyplot.plot(BEFOR_tim[-1], BEFOR_data[-1, ch], color='#dd1c77', marker='x', markersize=10,
                        label='contact_point (before)')
    #            for i in range(CH-1):
    ...
    if BEFOR_flag:
        ...
        for i in range(CH):
            try:
                popt, pconv = curve_fit(_line, BEFOR_tim[-rr - dr:-dr] - tim[0], BEFOR_data[-rr - dr:-dr, i],
                                        sigma=BEFOR_noise[-rr - dr:-dr, i])
            except:
                popt, pconv = curve_fit(_line, BEFOR_tim[:-dr] - tim[0], BEFOR_data[:-dr, i],
                                        sigma=BEFOR_noise[:-dr, i])
        ...
        if BEFOR_point[ch] < MIN_AVG[ch]:
            BEFOR_flag = False
        if plot and BEFOR_flag:
            pyplot.plot(tim, ((tim - tim[0]) * BEFOR_slopes[ch] + BEFOR_interc[ch] + BEFOR_DF[ch]), color='#dd1c77',
                        linestyle=':', label='linear regression (before)')
    ...
    if AFTER_flag:
        ...
        for i in range(CH):
            try:
                popt, pconv = curve_fit(_line, AFTER_tim[dr:rr + dr] - tim[0], AFTER_data[dr:rr + dr, i],
                                        sigma=AFTER_noise[dr:rr + dr, i])
            except:
        ...
        if AFTER_point[ch] < MIN_AVG[ch]:
            AFTER_flag = False
        if plot and AFTER_flag:
            pyplot.plot(tim, ((tim - tim[0]) * AFTER_slopes[ch] + AFTER_interc[ch] + AFTER_DF[ch]), color='#31a354',
                        linestyle=':', label='linear_regression (after)')
