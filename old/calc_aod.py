...
def _get_no2col(date, datapf, lat, lon, lvl=0):
    ...
    if os.path.exists(datapf + "no2/no2.dat"):
        ...
        for i in range(len(d[1:, 0])):
            if int(d[i + 1, 0]) == int(date.strftime("%Y%m%d")) and "%.1f" % (d[i + 1, 1]) == "%.1f" % (
            lat) and "%.1f" % (d[i + 1, 2]) == "%.1f" % (lon):
                return d[i + 1, 3]
    ...
    for f in flist:
        if f[19:23] + f[24:28] == date.strftime("%Y%m%d"):
            ...
            c = d.groups['HDFEOS'].groups['GRIDS'].groups['ColumnAmountNO2'].groups['Data Fields'].variables[
                    'ColumnAmountNO2'][:, :]
            d.close()
...
def _get_o3col(date, datapf, lat, lon, lvl=0):
    ...
    for f in flist:
        if f[19:23] + f[24:28] == date.strftime("%Y%m%d"):
            ...
            c = d.groups['HDFEOS'].groups['GRIDS'].groups['OMI Column Amount O3'].groups['Data Fields'].variables[
                    'ColumnAmountO3'][:, :]
            d.close()
...
def _h2ood(wvl, cosz, I0, eI0, aod440, aod870, I940, w440, w870, w940):
    ...
    am = cosz + a_H2O * ((90. - zen + b_H2O) ** (
        -c_H2O))  # h2o airmass  according to Kasten 1965    Kasten (Arch. Met. Geoph. Bioklim. B, Bd. 14, II.2, 206, 1965)
    am = 1. / am
...
def _E_AOD(date, datapf, sza, I, I0, eI0, aod, rod, o3OD, no2OD, h2ood, E_ext, E_ali, E_noi, e_h2o_od, P, dP=5.):
    dI = np.sqrt(
        (E_ali * I / (100.)) ** 2 + E_noi ** 2 + (0.02 * I) ** 2 + (0.01 * I) ** 2)  # E_ali, E_noise, _E_cal, E_extra
    ...
    rem[:, -1] = (4.361 * 10 ** (-5)) ** 2 + (1.764 * 10 ** (-5)) ** 2 + (4.76 * 10 ** (-5)) ** 2 + e_h2o_od[:,
                                                                                                    -1] ** 2  # dCO2,dCH4,DH20
