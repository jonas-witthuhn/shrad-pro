...
def shband_angle(a,
    ...
    if np.isscalar(r):
        ...
        Bt = np.tensordot(M, Bs, (
        1, 0))  ##Bt.shape : (3,len(r),len(a),len(phi),len(Db)) or  (3,len(r),len(phi),len(Db))
...
def check_collision(a, r, p, y, dr, dp, dy, zen, azi,
    ...
    gc.enable()
    if not np.isscalar(r):
        if not (len(r) == len(a) and len(r) == len(p) and len(r) == len(y) and len(r) == len(zen) and len(r) == len(
                azi)):
