...
def tilt_correction(tiltcfg):
    ...
    if not self.noINS:
        ...
        angerror=self._E_motion(self.channels,zen,self._calc_viewzen(roll,pitch,yaw,zen,azi,droll,dpitch,dyaw))
        ...
        if yaw==None and dyaw==None: # cant calculate the misaligned zenith angle
            ...
            angcors2=np.zeros((len(tim),len(self.channels))) #maximum correction factor -> radiometer is misaligned towards the sun
            for i in range(len(self.channels)):
                angcors2[:,i]=griddata(angzen,angcor[:,i],beta)
            drad=raw['rad'][:,:]/angcors2[:,:]
            raw['rad'][:,:]=raw['rad'][:,:]/angcors[:,:]
            angerror=np.abs(drad-raw['rad'][:,:])*100./raw['rad'][:,:]#[%]
            angerror+=self._E_motion(self.channels,zen,self._get_gamma(roll,pitch,azi,zen,azi,droll,dpitch,dyaw))
        else:
