    ...
    def _loadINS(date, datapf, filecfg, tiltcfg, lvl):
        ...
        if os.path.exists(fname):
            ...
            if len(dat[0, 1]) == 8:  # split seconds are missing
                ...
                datet = dat[:, 0] + 'T' + dat[:, 1]
            datet = np.core.defchararray.replace(np.array(datet, dtype='|U21'), '/',
                                                 '-')  # convert timestring to numpy format YYYY/MM//DD -> YYYY-MM-DD
    ...
    def _SMSmet2met(date, datapf, filecfg, lvl):
        ...
        SMSmet = np.loadtxt(SMSmet_fname, skiprows=4, delimiter=',',
                            converters={0: lambda s: (
                                        datetime.strptime(s.decode().strip('"'), "%Y-%m-%d %H:%M:%S") - datetime(1970,
                                                                                                                 1,
                                                                                                                 1)).total_seconds()})
...
def guv2pangaea(date, datapf, aod, pro, altitude):
    ...
    if len(pro['Iglo'][:, :].mask.shape) != 0:
        empty = (pro['Iglo'][:, :].data < 0) + (pro['Iglo'][:, :].data > 2000) + np.isnan(pro['Iglo'][:, :].data) + pro[
                                                                                                                        'Iglo'][
                                                                                                                    :,
                                                                                                                    :].mask
    else:
